...
Source0:	http://downloads.sourceforge.net/aepps/mod_%{mod_name}-%{version}.tar.gz
# Source0-md5:	1418187bf2b478ef2769271000f2b2c8
Source1:	http://downloads.sourceforge.net/aepps/epp-erd-20030122.tar.gz
...
install -d $RPM_BUILD_ROOT{%{_pkglibdir},%{_bindir},%{_sysconfdir},%{_examplesdir}/%{name}-%{version}}

libtool --mode=install install mod_%{mod_name}.la $RPM_BUILD_ROOT%{_pkglibdir}
rm -f $RPM_BUILD_ROOT%{_pkglibdir}/*.{l,}a

install -p epptelnet.pl $RPM_BUILD_ROOT%{_bindir}
cp -a examples/* $RPM_BUILD_ROOT%{_examplesdir}/%{name}-%{version}

echo 'LoadModule %{mod_name}_module modules/mod_%{mod_name}.so' > \
	$RPM_BUILD_ROOT%{_sysconfdir}/68_mod_%{mod_name}.conf
